...
class RequestsMock(ContextDecorator):
    ...
    def add(self, method: str, url: str, *, body: str | None = None, json: Any | None = None, status: int = 200) -> None:
        self._mocks.append(_Mock(method.upper(), url, status, body, json))

    def _dispatch(self, session: requests.Session, method: str, url: str, kwargs: Dict[str, Any]) -> requests.Response:
        for idx, mock in enumerate(self._mocks):
            if mock.method == method and mock.url == url:
                self.calls.append({"method": method, "url": url, "kwargs": kwargs})
                self._mocks.pop(idx)
                return requests.Response(mock.status, json_data=mock.json_data, text=mock.body)
        raise requests.RequestException(f"No mock registered for {method} {url}")
...
def add(method: str, url: str, *, body: str | None = None, json: Any | None = None, status: int = 200) -> None:
    if not _active:
